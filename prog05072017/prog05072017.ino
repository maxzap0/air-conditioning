...
//TODO функция: если сработало реле вентилятора, и через некоторе время не поднялись обороты, выключаем - компрессор.
...
#define time_fan1 220           //Время после которого при работе вентилятора включится компрессор 1 (ms)
#define time_fan2 220           //Время после которого при работе вентилятора включится компрессор 1 (ms)
...
t_real=temp_metr_in(35); 
t_out=temp_metr_out(45);  
//t_out=35;
...
B = bitRead (A_PORT, A_pin);  
A = bitRead (B_PORT, B_pin);
//Если нужно что бы энкодер крутился в другу сторону:

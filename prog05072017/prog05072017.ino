...
1100 - настройка втрого горячего вентилятора
1110 - температура со второго датчика температуры
1010 - индикация ошибок
0001 - настройка таблицы оборотв от температуры
*/
...
void rpm_hot_adjustment_manual() {
  ...
  } else if (rpm_array[temp_num]<=0) {
    rpm_array[temp_num]=0;
  }
  ...
  if (indicator==1) {                                                      //Если индикатор нажат
    ...
    rpm_array[temp_num]=encoder_read(rpm_array[temp_num], 1);              //Изменям значение оборотов ввыводиое на DAC
    if (  change_val(rpm_array[temp_num], rpm_array[temp_num], 0)   ) {    //Отображаем обороты, мигаем при изменении, и если изменились 
